...
int main(int argc,char *argv[])
{
   ...
   //cout<< "hello world";
	...
	if (argv[1][0]=='e'&&argc==4)
	{
		...
		ostringstream oss;
		int len;
		while((len = file.readsome(buf, 1024)) > 0)
		{
            oss.write(buf, len);
		}
		string data = oss.str();
		...
		while(now_poi<blockLen)
		{
		    ...
		    for (i=0;i<16;i++)
		    {
			    if(now_poi+i>blockLen)
				    t_block[i]=0;
			    else{
                    t_block[i]=data[now_poi+i];
                }
		    }
  ...
  else if (argv[1][0]=='d'&&(argc==4||argc==5))
	{
        if(argc == 4 ){
            cout<<"please add key file\n";
            return 0;
        }
        ifstream keyfile(argv[4],ios::in);
        char *keybuf = new char [keyLen];
        keyfile.read(keybuf,keyLen);
        for(i = 0; i < keyLen; i++){
            key[i] = keybuf[i];
        }
		ifstream file(argv[2],ios::binary | ios::in);
		...
		while(now_poi<blockLen)
	   {
			...
			for (i=0;i<16;i++)
			    t_block[i]=data[now_poi+i];
			AES_Decrypt(t_block, key, expandKeyLen);
			for (i=0;i<16;i++){
                char temp = t_block[i];
                data2 += temp;
            }
	...
	cout<<"Please enter e/d input ouput"<<endl;
}	
	AES_Done();
}
